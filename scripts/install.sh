...
setup_timezone_cn() {
  ...
  # 用户本地时间一致才便于排障。这只改时区显示，不碰 NTP 源与守护进程。
...
#
# 曾经写过「把上游换成国内源 + enable 守护进程」的逻辑，全部撤掉，原因：
#  1. 要适配的组合是 timesyncd / chrony / ntpsec / openntpd × 各发行版的 conf 布局。
#     Ubuntu 26.04 的 chrony.conf 还由 ucf 三方合并生成，安装器往里塞行会与
#     用户手改、发行版升级互相打架——这是我们不拥有的子系统。
#  2. 实测 22.04 / 24.04 / 26.04 的官方 chrony.conf 本来就带 confdir，绝大多数
#     机器根本不需要我们插手；之前误以为「24.04+ 无 confdir」是把某台测试机
#     被手改过的配置当成了发行版特性。
...
enable_services() {
  # 时区固定北京时间，便于日志排障。
  # ⚠️ 注意这里刻意不动 NTP 守护进程与上游源（理由见上方大段注释）：
